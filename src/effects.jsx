...
export function ExperienceControls(){
  ...
  return <div className="experience-controls" aria-label="Ovoz va rang sozlamalari"><button aria-pressed={sound} onClick={()=>{enabled=!sound;setSound(enabled);localStorage.setItem('sq_sound',enabled?'on':'off');if(master)master.gain.value=enabled?.18:0;if(enabled)playSound('arena')}} title="SFX ovoz effektlari">{sound?<Volume2 size={17}/>:<VolumeX size={17}/>}<span>SFX {sound?'yoqilgan':'o‘chiq'}</span></button><button aria-label={theme==='dark'?'Light rejimga o‘tish':'Dark rejimga o‘tish'} onClick={()=>setTheme(theme==='dark'?'light':'dark')} title="Dark / Light">{theme==='dark'?<Sun size={17}/>:<Moon size={17}/>}<span>{theme==='dark'?'Light':'Dark'}</span></button></div>
}
...
export function MotionScene(){return <div className="motion-scene" aria-hidden="true"><div className="scene-orbit"/><div className="scene-floor"/><div className="scene-mascot"><Avatar3D value="🤖" large/></div><div className="app-block block-word">W<small>Word</small></div><div className="app-block block-excel">X<small>Excel</small></div><div className="app-block block-ppt">P<small>Slides</small></div><div className="scene-key"><kbd>Ctrl</kbd><b>+</b><kbd>S</kbd></div><div className="scene-prompt"><span>✦</span><div><b>Yaxshi savol. Zo‘r natija.</b><small>Prompt engineering</small></div></div><div className="scene-label">BILIMINGIZNI KEYINGI BOSQICHGA OLIB CHIQING</div></div>}
