...
function raceState(race,current){
 ...
 if(current.localMode){const players=(current.playerIds||[]).map(id=>race.racers.find(item=>item.id===id));if(players.some(player=>!player))fail('Poygachilar topilmadi.');return {race:raceSummary(race),localMode:true,playerIds:current.playerIds,questions:players.map(player=>race.phase==='running'&&!race.winnerId?safeQuestion(race.questions[player.index]):null),feedbackByPlayer:current.feedbackByPlayer||{},csrf:'firebase',serverNow:Date.now()}}
